...
public class ExamController {
	...
	public ResponseEntity<?> DeleteExam(@PathVariable int examId) {
		...
		try {
			...
			throw new NotFoundException("Exam with id" + examId + "doesn't present or already deleted");
		}
	}

	// to get exam by userId
	@GetMapping("/getExamByUserId/{userId}")
	public ResponseEntity<?> getExamById(@PathVariable(name = "userId") Long userId) {
		
		List<Exam> exams = service.getExamByUserId(userId);
		return ResponseEntity.status(HttpStatus.OK).body(exams);
	}
}
